 ...
 */
class getBingUrl(val mView: IView) {
    private val BingURL = "https://cn.bing.com/HPImageArchive.aspx?" +
            "format=js&idx=0&n=1&mkt=zh_CN"
    private val ImageSizeUrl = "https://cn.bing.com/ImageResolution.aspx?w=${mView.getWidth()}" +
            "&h=${mView.getHeight()}&mkt=zh-CN"
    ...
    private fun getResizePic(fJsonStr: bingResult) : String {
        ...
        if (reSize != null) {
            Log.d("test okhttp request", "https://www.bing.com${regex
            ...
            mView.showMes("failed get fit Bing image size!!", 1)
            return "https://www.bing.com${fJsonStr.images[0].url}"
        }
    }
}
